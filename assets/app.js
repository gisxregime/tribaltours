(function () {
    ...
    function redirectToSignInIfRequired() {
        ...
        const file = path || 'explore.html';
        const next = file + String(window.location.search || '') + String(window.location.hash || '');
        window.location.href = 'sign-in.html?next=' + encodeURIComponent(next);
    }
    ...
    function initExplorePage() {
        ...
        function dbTourCardMarkup(tour) {
            ...
            return [
                ...
                '<a href="tour-preview.html?tour=', encodeURIComponent(String(tour.id || '')), '" class="rounded-full bg-[#C7A34A] px-4 py-2 text-sm font-semibold text-white hover:bg-[#B8923E]">View Tour</a>',
                '</div>',
                '</div>',
                '</article>'
            ].join('');
        ...
        if (createForm) {
            createForm.addEventListener('submit', function (event) {
                ...
                const requestPayload = {
                    ...
                    description: 'Tour request for ' + String(payload.title || 'a custom tour') + '.',
                    location: payload.location || 'Philippines',
                    region: payload.region || 'manila',
                    duration: payload.duration || (durationInput ? durationInput.value : '3'),
                    budget: payload.budget || (budgetInput ? budgetInput.value : payload.budgetMin || 0),
                    budgetMin: payload.budget || payload.budgetMin || (budgetInput ? budgetInput.value : 0),
                    budgetMax: payload.budget || payload.budgetMax || (maxBudget ? maxBudget.value : payload.budgetMin || 0),
                ...
                submitTourRequest(requestPayload).then(function (result) {
                    ...
                    setTimeout(function () {
                        window.location.href = 'my-posts.html' + (requestId ? '?request=' + encodeURIComponent(requestId) : '');
                    }, 450);
                }).catch(function (error) {
                    if (error && error.code === 'auth') {
                        showToast('Please sign in to create a request.', 'warning');
                        setTimeout(function () {
                            window.location.href = 'sign-in.html?next=' + encodeURIComponent('explore.html');
                        }, 600);
                        return;
                    }
        ...
        function renderStoredRequests(requests) {
            ...
            (Array.isArray(requests) ? requests : []).forEach(function (request) {
                ...
                card.innerHTML = [
                    ...
                    (request.selectedGuideId ? '<button class="btn-soft" data-open-conversation><i class="fa-regular fa-comments me-1"></i>Open Messages</button>' : ''),
                    '</div>',
                    '</div>'
                ].join('');
            ...
            requestGrid.addEventListener('click', function (event) {
                ...
                if (selectBtn) {
                    ...
                    submitGuideSelection(requestId, guideId, offerAmount).then(function (result) {
                        ...
                        const redirect = result && result.redirect ? String(result.redirect) : 'messages.html';
                        setTimeout(function () {
