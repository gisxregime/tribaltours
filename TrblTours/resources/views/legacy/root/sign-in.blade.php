    ...
    <main class="auth-shell" aria-label="TrblTours authentication">
        ...
        <section class="auth-panel">
            <div
                ...
                id="authApp"
                data-auth-context="signin"
                data-login-url="{{ route('login', absolute: false) }}"
                data-forgot-url="{{ route('password.email', absolute: false) }}"
                data-csrf-token="{{ csrf_token() }}"
                data-signin-email="{{ old('email', '') }}"
                data-signin-role="{{ old('role', 'tourist') }}"
                data-signin-remember="{{ old('remember') ? '1' : '0' }}"
                data-signin-message="{{ session('status', '') }}"
                data-signin-error="{{ $errors->first('email') ?: $errors->first('password') }}"
            ></div>
        </section>
    </main>
